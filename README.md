...
有两个任务可供选择，**run_babel** 任务只会处理 "*.es6" 文件，将其中的 ES6 语法部分转译成多数浏览器支持的等效代码，供浅度尝试 ES6 语法的项目使用。

使用 **rollup_bundle** 任务的话，配合其中的 `babel`、`common-js`、`vue` 等插件，能实现 ES6 转译和脚本打包的完整流程。
